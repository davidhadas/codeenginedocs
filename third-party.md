...
| Guard | [Guard](https://github.com/IBM/workload-security-guard/){: external} is an open source,  workload runtime security solution, well equipped to protect serverless services. In Code Engine, Guard is presently offered as a security layer protecting the deployed services. Guard  micro rules whitelisting allows control over every aspect of the the service ingress. A Guard Service can be added to ease micro rule management by auto-learning micro rules for each protected service. |
